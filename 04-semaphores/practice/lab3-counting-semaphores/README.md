...
1. เมื่อ Producers มากกว่า Resources จะเกิดอะไรขึ้น?
    Task (Producer) บางตัวจะ block รอ Resource จนกว่า semaphore ถูก give กลับมา → task ที่มาก่อนจะได้ใช้ resource ก่อน ตามลำดับ FIFO ของ semaphore
2. Load Generator มีผลต่อ Success Rate อย่างไร?
    • ถ้า load สูง → task จำนวนมากรอ resource → success rate ลดลง
    • ถ้า load ต่ำ → resource สามารถให้บริการ task ได้ทัน → success rate สูง
3. Counting Semaphore จัดการ Resource Pool อย่างไร?
    • Semaphore นับจำนวน resource ที่ว่าง (count)
    • take() → ลดค่า count → block ถ้า count = 0
    • give() → เพิ่มค่า count → ปลุก task ที่รอ resource
