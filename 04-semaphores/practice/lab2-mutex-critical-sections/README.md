...
1. เมื่อไม่ใช้ Mutex จะเกิด data corruption หรือไม่?
    เกิดได้แน่นอน หากมีหลาย task เข้าถึง shared resource พร้อมกัน → ข้อมูลอาจถูกเขียนทับหรืออ่านผิดพลาด
2. Priority Inheritance ทำงานอย่างไร?
    เมื่อ task priority ต่ำ ถือ mutex และ task priority สูง รอ mutex
    → task ต่ำจะ ถูกยก priority ชั่วคราว ให้สูงขึ้นเท่ากับ task ที่รอ เพื่อปล่อย resource เร็วขึ้น ลดปัญหา priority inversion
3. Task priority มีผลต่อการเข้าถึง shared resource อย่างไร?
    Task priority สูงจะรอ task ต่ำ หากไม่มี priority inheritance → อาจเกิด priority inversion
    ใช้ mutex + priority inheritance → resource ถูกปล่อยเร็วขึ้น task สูงได้ทำงานทันที
