...
1. เมื่อ give semaphore หลายครั้งติดต่อกัน จะเกิดอะไรขึ้น?
    ...
    • Binary Semaphore → ค่า max = 1 → give หลายครั้งต่อเนื่อง ไม่เพิ่มค่าเกิน 1
    • Counting Semaphore → ค่าจะเพิ่มขึ้นตามจำนวน give (จนถึง max count)
2. ISR สามารถใช้ `xSemaphoreGive` หรือต้องใช้ `xSemaphoreGiveFromISR`?
    ต้องใช้ xSemaphoreGiveFromISR เพราะ ISR ไม่สามารถเรียกฟังก์ชันที่ block หรือใช้ scheduler ได้โดยตรง
    ฟังก์ชันนี้ออกแบบมาให้ safe ใน interrupt context
3. Binary Semaphore แตกต่างจาก Queue อย่างไร?
    • Binary Semaphore ใช้ สัญญาณระหว่าง task/ISR เพื่อ sync หรือ signaling
    • Queue ใช้ เก็บข้อมูลจริง (data) พร้อม FIFO
    → Semaphore ไม่เก็บข้อมูล แค่บอกว่าพร้อมหรือไม่
