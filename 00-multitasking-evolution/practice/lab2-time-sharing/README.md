...
1. Time slice ขนาดไหนให้ประสิทธิภาพดีที่สุด? เพราะอะไร?
    ขนาด ปานกลาง (เช่น 5–10 ms) ให้ประสิทธิภาพดีที่สุด ถ้า slice สั้น → ระบบตอบสนองไวแต่เปลืองเวลาเปลี่ยน context 
    ถ้า slice ยาว → ระบบตอบสนองช้าแต่ overhead ต่ำ
    ขนาดปานกลางจึงเป็นจุดสมดุลระหว่าง “ความไวในการตอบสนอง” กับ “ประสิทธิภาพการใช้ CPU”
2. ปัญหาอะไรที่เกิดขึ้นเมื่อ time slice สั้นเกินไป?
    เกิด context switching บ่อยเกิน →  CPU เสียเวลาเปลี่ยนงานมาก (overhead สูง) ประสิทธิภาพรวมลดลง
3. ปัญหาอะไรที่เกิดขึ้นเมื่อ time slice ยาวเกินไป?
    ระบบ ตอบสนองช้า ต่อเหตุการณ์ภายนอก เช่น ปุ่มหรือ sensor งานที่มี priority ต่ำต้อง รอนานกว่าจะได้ทำงาน ทำให้ระบบเหมือน “pseudo single task” มากขึ้น
4. Context switching overhead คิดเป็นกี่เปอร์เซ็นต์ของเวลาทั้งหมด?
    โดยทั่วไปอยู่ที่ประมาณ 1–5 % ของเวลาทั้งหมด (ขึ้นกับขนาดของ task และความถี่ในการสลับ) ถ้า time slice สั้นมาก หรือมีหลาย task → ค่า overhead จะพุ่งสูงเกิน 10 % ได้
5. งานไหนที่ได้รับผลกระทบมากที่สุดจากการ time-sharing?
    งานที่ต้องการเวลาประมวลผลต่อเนื่อง (CPU-bound) เช่น การคำนวณซับซ้อน การประมวลผลภาพ / เสียง การเข้ารหัสข้อมูล เพราะถูกแบ่งเวลาให้ task อื่นบ่อย ทำให้ทำงานไม่ต่อเนื่อง
