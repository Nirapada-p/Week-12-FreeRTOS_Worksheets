...
1. ไฟล์ใดบ้างที่จำเป็นสำหรับโปรเจกต์ ESP-IDF ขั้นต่ำ?
    main/main.c – โค้ดหลักของโปรแกรม
    • CMakeLists.txt – ไฟล์กำหนดการ build
    • sdkconfig – ค่าการตั้งค่าของโปรเจกต์
    • idf.py ใช้เป็นเครื่องมือ build และ flash
2. ความแตกต่างระหว่าง `hello_esp32.bin` และ `hello_esp32.elf` คืออะไร?
    • .elf เป็นไฟล์ Executable and Linkable Format มีข้อมูล debug และ symbol ครบ
    • .bin เป็นไฟล์ ไบนารีที่พร้อมแฟลชลงบอร์ด ไม่มีข้อมูล debug
3. คำสั่ง `idf.py set-target` ทำอะไร?
    ใช้ กำหนดชนิดของชิป ESP ที่จะ build เช่น esp32, esp32s3, esp32c3 เพื่อให้ระบบเลือก SDK และการคอมไพล์ที่ถูกต้อง
4. โฟลเดอร์ `build/` มีไฟล์อะไรบ้าง?
    เป็นที่เก็บผลลัพธ์หลังคอมไพล์ เช่น
    • .bin (ไฟล์สำหรับแฟลช)
    • .elf (ไฟล์ดีบัก)
    • project_description.json
    • log และไฟล์ temporary อื่น ๆ
5. การใช้ `vTaskDelay()` แทน `delay()` มีความสำคัญอย่างไร?
    vTaskDelay() เป็นของ FreeRTOS ที่ ปล่อย CPU ให้ task อื่นทำงาน ได้ในช่วงรอเวลา
    ในขณะที่ delay() จะ บล็อก CPU ทั้งหมด ทำให้ระบบไม่สามารถทำงานหลาย task พร้อมกันได้
