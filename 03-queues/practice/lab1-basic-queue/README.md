...
1. เมื่อ Queue เต็ม การเรียก `xQueueSend` จะเกิดอะไรขึ้น?
    ขึ้นอยู่กับ timeout:
    • ถ้า timeout > 0 → task block รอจนกว่าจะมีพื้นที่ว่าง
    • ถ้า timeout = 0 → ส่งไม่สำเร็จทันที (return errQUEUE_FULL)
2. เมื่อ Queue ว่าง การเรียก `xQueueReceive` จะเกิดอะไรขึ้น?
    ขึ้นอยู่กับ timeout:
    • ถ้า timeout > 0 → task block รอจนมีข้อมูลเข้ามา
    • ถ้า timeout = 0 → รับไม่สำเร็จทันที (return errQUEUE_EMPTY)
3. ทำไม LED จึงกะพริบตามการส่งและรับข้อความ?
    เพราะ task ส่งและ task รับ Queue ทำงานร่วมกัน
    LED ถูก toggle ใน task รับข้อมูล → จึงกะพริบตามการส่งข้อความเข้ามาใน Queue
