...
1. ในทดลองที่ 2 เกิดอะไรขึ้นกับ Queue?
    Queue รับข้อมูลมากกว่าที่ส่งหรือเต็มชั่วคราว → task ที่ส่งต้อง block รอ หรือส่งไม่สำเร็จ ขึ้นอยู่กับ timeout
2. ในทดลองที่ 3 ระบบทำงานเป็นอย่างไร?
    ระบบทำงานแบบ Producer-Consumer:
    • Task ส่งข้อมูล → เข้าสู่ Queue
    • Task รับข้อมูล → ประมวลผลหรือ toggle LED
    → ทำให้การทำงาน ซิงโครไนซ์ระหว่าง task หลายตัว
3. Load Balancer แจ้งเตือนเมื่อไหร่?
    เมื่อ task หนึ่งหรือหลาย task ถูกใช้งานหนักเกินไป หรือ Queue เต็ม/รอค้าง → ระบบตรวจสอบแล้วแจ้งเตือนเพื่อให้ผู้ใช้หรือ task อื่นรับมือได้ทันที
