...
1. Processor Task รู้ได้อย่างไรว่าข้อมูลมาจาก Queue ไหน?
    ใช้ Queue Set API:
    xQueueSelectFromSet() จะบอก handle ของ Queue ที่มีข้อมูล พร้อมให้ task ประมวลผลต่อได้
2. เมื่อหลาย Queue มีข้อมูลพร้อมกัน เลือกประมวลผลอันไหนก่อน?
    Queue Set จะ ส่ง Queue ที่มีข้อมูลมาก่อน (FIFO ของ Set) หรือสามารถเลือกตาม priority ของ task ที่รอคิวอยู่ ขึ้นกับการตั้งค่า
3. Queue Sets ช่วยประหยัด CPU อย่างไร?
    Task สามารถ block รอหลาย queue พร้อมกัน แทนที่จะวนเช็คทีละ queue → ลดการใช้ CPU และ overhead ของ polling task
