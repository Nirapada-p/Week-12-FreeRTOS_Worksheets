...
1. **Watchdog Design**: เหตุใดต้องใช้ separate timer สำหรับ feeding watchdog?
    เพื่อให้ ระบบตรวจจับ task ค้างหรือ loop หยุดทำงาน ได้แม่นยำ
    Timer แยกช่วยให้ watchdog ไม่ขึ้นอยู่กับ task หลัก และสามารถ reset ระบบทันทีเมื่อเกิด deadlock
2. **Pattern Timing**: อธิบายการเลือก Timer Period สำหรับแต่ละ pattern
    Timer period ต้อง สอดคล้องกับความถี่ที่ต้องการ เช่น
    • LED blink pattern → period = 100–500 ms
    • Sensor read pattern → period = sampling rate ของ sensor
    → ช่วยให้ระบบทำงาน แม่นยำและ predictable
3. **Sensor Adaptation**: ประโยชน์ของ Adaptive Sampling Rate คืออะไร?
    • ลด power consumption โดยลดความถี่เมื่อความเปลี่ยนแปลงน้อย
    • เพิ่ม responsiveness เมื่อเกิด event หรือสัญญาณเปลี่ยนแปลงเร็ว
    • ทำให้ระบบ efficient และแม่นยำ ในการเก็บข้อมูล
4. **System Health**: metrics ใดบ้างที่ควรติดตามในระบบจริง?
    • CPU utilization
    • Memory usage / Stack high-water mark
    • Task response time / latency
    • Queue/semaphore status
    • Watchdog triggers / system resets
    → ช่วยประเมินความเสถียรและประสิทธิภาพของระบบ
